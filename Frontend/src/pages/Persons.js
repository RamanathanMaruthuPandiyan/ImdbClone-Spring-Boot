...
const Persons = () => {
    ...
    const queryFunction = (isFilter) => {
        let limit = pagination.limit || 15;
        let page = 0;

        if (isFilter) {
            setCurrentPage(1);
            paginationQuery = { page, size: limit };
            setSelectedDataList(limit);
        } else {
            limit = pagination.limit || paginationDataLimit.limit || 15;
            page = Math.floor((pagination.skip || 0) / limit);
            paginationQuery = { page, size: limit };
        }

        setPaginationDataLimit({ limit }); // still keep `limit` separately if needed
            ...
            if (key === 'filter' && typeof value === 'object') {
                for (const subKey in value) {
                    ...
                    if (Array.isArray(subValue)) {
                        subValue.forEach(v => params.append(`${subKey}`, v));
                    } else {
    ...
    const getTableData = async (isFilter) => {
        const baseUrl = `persons/pagination`;
        setLoading(true);
        try {
            let query = {};
    ...
    const getGenderEnum = async () => {
        const url = `enums/gender`;
        setLoading(true);
        try {
            let result = await getData(url);
            let genderOption = Object.entries(result.gender.label).map(([key, value]) => ({
                value: key,
                label: value
            }));
                    ...
                    <div className="row mt-3">
                        <div className="col">
                                ...
                                <tbody>
                                    {tableData.length > 0 ? (
                                        tableData.map((record, i) => (
                                            <tr key={i}>
                                                <td>{record.name}</td>
                                                <td>{record.gender ? (genderEnum && Object.keys(genderEnum).length) ? genderEnum.gender.label[record.gender] : "-" : "-"}</td>
                                                <td>{record.dob ? record.dob : "-"}</td>
                                                <td>{record.bio ? truncateText(record.bio, 15) : "-"}</td>
                                                <td>
                                                    {record.producedMovie && record.producedMovie.length > 0 ? (
                                                        <ul style={{ paddingLeft: "20px", margin: 0 }}>
                                                            {record.producedMovie.map((person, index) => (
                                                ...
                                                <td>
                                                    {record.actedMovies && record.actedMovies.length > 0 ? (
                                                        <ul style={{ paddingLeft: "20px", margin: 0 }}>
                                                            {record.actedMovies.map((person, index) => (
                                                                <li key={index}>{person}</li>
                                                            ))}
                                                        </ul>
                                                    ) : (
                                                ...
                                                <td className="action-dropdown">
                                                    <div className="dropdown" onClick={() => getActionItems(record.id)}>
                                                        ...
                                                        <ul className="dropdown-menu dropdown-menu-end">
                                                            {actionItems.includes(actionItemEnums?.actionItems?.EDIT) && (
                                                                <li>
                                                            ...
                                                            {actionItems.includes(actionItemEnums?.actionItems?.DELETE) && (
                                                                <li>
                            ...
                            <Pagination
                                ...
                                paginationDataLimit={{ limit: response.size }}
                                response={{ totalRecords: response.totalElements }}
