...
const Jobs = () => {
    ...
    const [enumJobNames, setEnumJobNames] = useState([]);
    const [enumJobStatus, setEnumJobStatus] = useState([]);
    ...
    const getEnumJobNames = async () => {
        const url = `enums/jobNames`;
        setLoading(true);
        try {
            let result = await getData(url);
            setEnumJobNames(result);
        } catch (e) {
            toast.error(e.response.data.message);
        } finally {
            setLoading(false);
    ...
    const getEnumJobStatus = async () => {
        const url = `enums/jobStatus`;
        setLoading(true);
        try {
            let result = await getData(url);
            setEnumJobStatus(result);
        } catch (e) {
            toast.error(e.response.data.message);
        } finally {
            setLoading(false);
                                ...
                                <tbody>
                                    {tableData.length > 0 ? (
                                        tableData.map((record, i) => (
                                            <tr key={i}>
                                                <td>{record.name ? (enumJobNames && Object.keys(enumJobNames).length) ? enumJobNames.jobNames.label[record.name] : "-" : "-"}</td>
                                                ...
                                                >
                                                    {record.status ? (enumJobStatus && Object.keys(enumJobStatus).length) ? enumJobStatus.jobStatus.label[record.status] : "-" : "-"}</span>
                                                </td>
